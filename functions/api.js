/**
 * DigitalSaudi · /api · v4.0
 * Prayer · Weather · Holidays · Qibla · Scores
 */
const CORS = {
...
async function prayer(city){
  ...
  const now=new Date(),d=`${now.getDate()}-${now.getMonth()+1}-${now.getFullYear()}`;
  try{
    ...
    const data=await r.json(),t=data.data?.timings||{},h=data.data?.date?.hijri||{};
    return{city:c.name,ar:c.ar,date:data.data?.date?.readable,
      hijri:{day:h.day,monthAr:h.month?.ar,month:h.month?.en,year:h.year,isRamadan:parseInt(h.month?.number||0)===9},
      timings:{Fajr:t.Fajr,Sunrise:t.Sunrise,Dhuhr:t.Dhuhr,Asr:t.Asr,Maghrib:t.Maghrib,Isha:t.Isha}};
  }catch(e){
    return{city:c.name,ar:c.ar,date:new Date().toDateString(),hijri:{isRamadan:false},
      timings:{Fajr:'04:45',Sunrise:'06:10',Dhuhr:'12:15',Asr:'15:30',Maghrib:'18:45',Isha:'20:15'},offline:true};
  }
}

async function weather(city,all){
  if(all){
  ...
  try{
    ...
    const d=await r.json(),cur=d.current||{},daily=d.daily||{},w=WX(cur.weather_code||0);
...
function holidays(){
  ...
  return[
    {name:'Founding Day',ar:'يوم التأسيس',date:`${y}-02-22`,flag:'🇸🇦'},
...
export async function onRequestGet(context){
  ...
  try{
    switch(type){
      case 'prayer':data=await prayer(city);cache=3600;break;
      case 'weather':data=await weather(city,all);cache=1800;break;
      case 'holidays':data={holidays:holidays()};cache=3600;break;
      case 'qibla':data=qibla(lat,lng);cache=86400;break;
      default:data={status:'ok',app:'DigitalSaudi',v:'4.0'};
    }
